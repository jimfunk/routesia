...
class PoolDefinition:
    ...
    def __init__(self, value: str):
        if not self.POOL_RE.fullmatch(value):
            raise ValueError("pool definition does not match expected <start_address>-<end_address>")
...
class DHCPServerCLI(Provider):
    def __init__(self, cli: CLI, rpc: RPCClient):
        ...
        self.cli.add_argument_completer("option-definition-name", self.complete_option_definition_names)
        ...
        self.cli.add_argument_completer("client-class-option-definition", self.complete_client_class_option_definitions)
        self.cli.add_argument_completer("client-class-option-name", self.complete_client_class_option_names)
        self.cli.add_argument_completer("client-class-option-code", self.complete_client_class_option_codes)
        self.cli.add_argument_completer("subnet", self.complete_subnets)
        self.cli.add_argument_completer("pool", self.complete_pools)
        self.cli.add_argument_completer("subnet-option-name", self.complete_subnet_option_names)
        self.cli.add_argument_completer("subnet-option-code", self.complete_subnet_option_codes)
        self.cli.add_argument_completer("subnet-hardware-reservation", self.complete_subnet_hardware_reservations)
        self.cli.add_argument_completer("subnet-relay-address", self.complete_subnet_relay_addresses)

        self.cli.add_command("dhcp server v4 config show", self.show_config)
        self.cli.add_command("dhcp server v4 config interface list", self.list_interfaces)
        self.cli.add_command("dhcp server v4 config interface add :interface", self.add_interface)
        self.cli.add_command("dhcp server v4 config interface delete :interface", self.delete_interface)
        self.cli.add_command("dhcp server v4 config global-settings update @renew-timer @rebind-timer @valid-lifetime @next-server", self.update_global_settings)
        self.cli.add_command("dhcp server v4 config global-settings show", self.show_global_settings)
        self.cli.add_command("dhcp server v4 config option-definition list", self.list_option_definitions)
        self.cli.add_command("dhcp server v4 config option-definition add :name :code :type @space @array @record-types @encapsulate", self.add_option_definition)
        self.cli.add_command("dhcp server v4 config option-definition update :option-name @option-code @type @space @array @record-types @encapsulate", self.update_option_definition)
        self.cli.add_command("dhcp server v4 config option-definition delete :option-name", self.delete_option_definition)
        self.cli.add_command("dhcp server v4 config option list", self.list_options)
        self.cli.add_command("dhcp server v4 config option add @name @code @data", self.add_option)
        self.cli.add_command("dhcp server v4 config option update @option-name @option-code @data", self.update_option)
        self.cli.add_command("dhcp server v4 config option delete @option-name @option-code", self.delete_option)
        self.cli.add_command("dhcp server v4 config client-class list", self.list_client_classes)
        self.cli.add_command("dhcp server v4 config client-class add :name :test @next-server", self.add_client_class)
        self.cli.add_command("dhcp server v4 config client-class update :client-class @test @next-server", self.update_client_class)
        self.cli.add_command("dhcp server v4 config client-class delete :client-class", self.delete_client_class)
        self.cli.add_command("dhcp server v4 config client-class option-definition list :client-class", self.list_client_class_option_definitions)
        self.cli.add_command("dhcp server v4 config client-class option-definition add :client-class :name :code :type @space @array @record-types @encapsulate", self.add_client_class_option_definition)
        self.cli.add_command("dhcp server v4 config client-class option-definition update :client-class :client-class-option-definition @code @type @space @array @record-types @encapsulate", self.update_client_class_option_definition)
        self.cli.add_command("dhcp server v4 config client-class option-definition delete :client-class :client-class-option-definition", self.delete_client_class_option_definition)
        self.cli.add_command("dhcp server v4 config client-class option list :client-class", self.list_client_class_options)
        self.cli.add_command("dhcp server v4 config client-class option add :client-class @name @code @data", self.add_client_class_option)
        self.cli.add_command("dhcp server v4 config client-class option update :client-class @client-class-option-name @client-class-option-code @data", self.update_client_class_option)
        self.cli.add_command("dhcp server v4 config client-class option delete :client-class @client-class-option-name @client-class-option-code", self.delete_client_class_option)
        self.cli.add_command("dhcp server v4 config subnet list", self.list_subnets)
        self.cli.add_command("dhcp server v4 config subnet add :address @use-ipam @next-server", self.add_subnet)
        self.cli.add_command("dhcp server v4 config subnet update :subnet @use-ipam @next-server", self.update_subnet)
        self.cli.add_command("dhcp server v4 config subnet delete :subnet", self.delete_subnet)
        self.cli.add_command("dhcp server v4 config subnet pool add :subnet :pool-definition", self.add_pool)
        self.cli.add_command("dhcp server v4 config subnet pool delete :subnet :pool", self.delete_pool)
        self.cli.add_command("dhcp server v4 config subnet option list :subnet", self.list_subnet_options)
        self.cli.add_command("dhcp server v4 config subnet option add :subnet @name @code @data", self.add_subnet_option)
        self.cli.add_command("dhcp server v4 config subnet option update :subnet @subnet-option-name @subnet-option-code @data", self.update_subnet_option)
        self.cli.add_command("dhcp server v4 config subnet option delete :subnet @subnet-option-name @subnet-option-code", self.delete_subnet_option)
        self.cli.add_command("dhcp server v4 config subnet reservation list :subnet", self.list_subnet_reservations)
        self.cli.add_command("dhcp server v4 config subnet reservation add :subnet :hardware-address :ip_address", self.add_subnet_reservation)
        self.cli.add_command("dhcp server v4 config subnet reservation update :subnet :subnet-hardware-reservation :ip_address", self.update_subnet_reservation)
        self.cli.add_command("dhcp server v4 config subnet reservation delete :subnet :subnet-hardware-reservation", self.delete_subnet_reservation)
        self.cli.add_command("dhcp server v4 config subnet relay-address add :subnet :ip-address", self.add_relay_address)
        self.cli.add_command("dhcp server v4 config subnet relay-address delete :subnet :relay-address", self.delete_relay_address)
    ...
    async def complete_option_definition_names(self, **args):
    ...
    async def complete_option_names(self, **args):
        config = await self.rpc.request("dhcp/server/v4/config/get")
        completions = []
    ...
    async def complete_option_codes(self, **args):
        config = await self.rpc.request("dhcp/server/v4/config/get")
        completions = []
        for option in config.option:
    ...
    async def complete_client_classes(self, **args):
    ...
    async def complete_client_class_option_definitions(self, **args):
        config = await self.rpc.request("dhcp/server/v4/config/get", None)
        completions = []
        client_class_name = args.get("client-class")
        for client_class in config.client_class:
            if client_class.name == client_class_name:
                for option_definition in client_class.option_definition:
                    completions.append(option_definition.name)
                break
        return completions

    async def complete_client_class_option_names(self, **args):
        config = await self.rpc.request("dhcp/server/v4/config/get", None)
        completions = []
        client_class_name = args.get("client-class")
        for client_class in config.client_class:
    ...
    async def complete_client_class_option_codes(self, **args):
        config = await self.rpc.request("dhcp/server/v4/config/get", None)
        completions = []
        client_class_name = args.get("client-class")
        for client_class in config.client_class:
    ...
    async def complete_subnets(self, **args):
        config = await self.rpc.request("dhcp/server/v4/config/get", None)
        completions = []
    ...
    async def complete_pools(self, **args):
        config = await self.rpc.request("dhcp/server/v4/config/get", None)
        completions = []
        subnet_address = args.get("subnet")
        for subnet in config.subnet:
            if subnet.name == subnet_address:
                for pool in subnet.pool:
                    completions.append(pool)
                break
        return completions

    async def complete_subnet_option_names(self, **args):
        config = await self.rpc.request("dhcp/server/v4/config/get", None)
        completions = []
    ...
    async def complete_subnet_option_codes(self, **args):
        config = await self.rpc.request("dhcp/server/v4/config/get", None)
        completions = []
        subnet_name = args.get("subnet")
        for subnet in config.subnet:
    ...
    async def complete_subnet_hardware_reservations(self, **args):
        config = await self.rpc.request("dhcp/server/v4/config/get", None)
        completions = []
        subnet_name = args.get("subnet")
        for sn in config.subnet:
            if sn.address == subnet_name:
                for reservation in sn.reservation:
                    completions.append(reservation.hardware_address)
                break
        return completions

    async def complete_subnet_relay_addresses(self, **args):
        config = await self.rpc.request("dhcp/server/v4/config/get", None)
        completions = []
        subnet_name = args.get("subnet")
        for subnet in config.subnet:
    ...
    async def delete_option_definition(self, option_name):
        option_definition = dhcp_server_pb2.OptionDefinition()
    ...
    async def add_option(self, name = None, code: UInt32 = None, data = None):
    ...
    async def update_option(self, option_name = None, option_code: UInt32 = None, data = None):
        if (option_name is None and option_code is None) or (option_name is not None and option_code is not None):
    ...
    async def delete_option(self, option_name = None, option_code: UInt32 = None):
        if (option_name is None and option_code is None) or (option_name is not None and option_code is not None):
            raise InvalidArgument("one of option_name or option_code must be specified")
    ...
    async def add_client_class(self, name, test, next_server = None):
    ...
    async def update_client_class(self, client_class, test = None, next_server: IPv4Address = None):
        config = await self.rpc.request("dhcp/server/v4/config/get", None)
        item = None
    ...
    async def delete_client_class(self, client_class):
        item = dhcp_server_pb2.ClientClass()
        item.name = client_class
        await self.rpc.request(
            "dhcp/server/v4/config/client_class/delete", item
        )

    async def list_client_class_option_definitions(self, client_class):
        config = await self.rpc.request("dhcp/server/v4/config/get", None)
    ...
    async def update_client_class_option_definition(
        self,
        client_class,
        client_class_option_definition,
        code: UInt32 = None,
        type = None,
        space = None,
        array: bool = None,
        ...
        for od in item.option_definition:
            if od.name == client_class_option_definition:
                ...
                break
        if not option_definition:
            raise InvalidArgument("No such option-definition: %s" % client_class_option_definition)
        ...
        await self.rpc.request(
            "dhcp/server/v4/config/client_class/option_definition/update", client_class_item
        )

    async def delete_client_class_option_definition(self, client_class, client_class_option_definition):
        item = dhcp_server_pb2.ClientClass()
        item.name = client_class
        option_definition = item.option_definition.add()
    ...
    async def list_client_class_options(self, client_class):
        config = await self.rpc.request("dhcp/server/v4/config/get", None)
    ...
    async def delete_client_class_option(
        ...
        await self.rpc.request(
            "dhcp/server/v4/config/client_class/option/delete", item
        )
    ...
    async def delete_subnet(self, subnet: IPv4Interface):
        subnet = dhcp_server_pb2.DHCPv4Subnet()
    ...
    async def list_subnet_options(self, subnet: IPv4Interface):
        config = await self.rpc.request("dhcp/server/v4/config/get", None)
    ...
    async def update_subnet_option(
        self,
        subnet: IPv4Interface,
        subnet_option_name = None,
        subnet_option_code: UInt32 = None,
        data = None,
    ):
        field = None
        ...
        updated_option.CopyFrom(option)
        updated_option.data = data
        await self.rpc.request("dhcp/server/v4/config/subnet/option/update", updated_subnet)
    ...
    async def delete_subnet_option(
        ...
        if (subnet_option_name is None and subnet_option_code is None) or (subnet_option_name is not None and subnet_option_code is not None):
            raise InvalidArgument("one of subnet_option_name or subnet_option_code must be specified")
        ...
        await self.rpc.request("dhcp/server/v4/config/subnet/option/delete", subnet_item)

    async def list_subnet_reservations(self, subnet: IPv4Interface):
        config = await self.rpc.request("dhcp/server/v4/config/get", None)
    ...
    async def add_subnet_reservation(
        ...
        await self.rpc.request(
            "dhcp/server/v4/config/subnet/reservation/add", item
        )
    ...
    async def update_subnet_reservation(
        ...
        for res in subnet_item.reservation:
            if res.hardware_address == str(subnet_hardware_reservation):
                ...
                break
        if not reservation:
            raise InvalidArgument(f"No such reservation: {subnet_hardware_reservation} in subnet {subnet}")
    ...
    async def delete_subnet_reservation(
        ...
        await self.rpc.request(
            "dhcp/server/v4/config/subnet/reservation/delete", sn
        )
    ...
    async def show_subnet_leases(self, subnet: IPv4Interface):
        item = dhcp_server_pb2.DHCPv4Subnet()
        item.address = str(subnet)
